...
const ExperienceCard: React.FC<ExperienceCardProps> = ({ experience }) => {
  return (
    <div className="group relative overflow-hidden rounded-xl border border-border/80 bg-background/90 p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-primary/35 hover:shadow-xl sm:p-6">
      <div className="absolute inset-y-0 left-0 w-1 bg-gradient-to-b from-primary via-sky-500 to-emerald-500 opacity-80" />
      <div className="flex items-start gap-3 sm:gap-4">
        {experience.logo && (
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg border border-primary/20 overflow-hidden bg-primary/5 flex-shrink-0">
            <Image
              src={experience.logo}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-3 sm:mt-4 flex flex-wrap gap-1">
            {experience.skills.slice(0, 2).map((skill, index) => (
              <span
                key={index}
            className="inline-flex items-center rounded-full border border-border bg-muted px-2 py-1 text-xs font-medium text-muted-foreground"
              >
                {skill}
              </span>
            ))}
